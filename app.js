...
const App = (function(){
  ...
  function runPendingFile(){ const p=pendingFile; pendingFile=null; if(!p||!VAULT) return;
    const d=Date.now()-p.at; if(d<0||d>PENDING_FILE_MS){ toast(tr('imp.expired'),{ms:8000}); return; }   // d<0: Uhr zurückgestellt → nicht unbegrenzt gültig (Audit Tresor run-6 / Alien Pass run-9)
